function [model, species_list, errstr] = eq2mat_hard(r); % takes r, the mechanism string, and finds model
% eq2mat_hard;
%
...
try
   ...
   coeff = {};
   % for each reaction
	for k=1:nr % for the length of the mechanism string
	   spc_k = spc{k};% num of species
      coeff_k = ones(length(spc_k),1); % num of coeff
      % for each species
   ...
   % how many unique species, total
	...
	sp_all = sp_all(:);
	[uniq_sp,ix,xinall] = unique(sp_all);	% xinall = order of uniq_sp in sp_all       uinq_sp is all unique species in order, xinall is index that: sp_all = uniq_sp(xinall)
	ns = length(uniq_sp);
	...
	for i=1:nr % for each row
	   spci = spc{i};
	   coeffi = coeff{i}; % numeric coefficients in the row
	   for j=1:length(spci)
	      k = k+1;
         N(i,xinall(k)) = N(i,xinall(k)) + coeffi(j); % N will be # of rows/reactions by # of unique species puts numerical coefficient into N
         if coeffi(j) < 0
            % build R - reactant coefficient matrix
            R(i,xinall(k)) = R(i,xinall(k)) + coeffi(j); % R will be # of rows/reactions by # of unique species only puts negaive numerical coefficints into N
         elseif coeffi(j) > 0
