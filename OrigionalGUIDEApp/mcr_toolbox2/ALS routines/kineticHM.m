...
function edit_mech_Callback(hObject, eventdata, handles) % When the mechanism is edited
% hObject    handle to edit_mech (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
modelEscrit=get(hObject,'String'); % get the string of mechnism entered
hard.model.modelEscrit = cellstr(modelEscrit); % mechanism string 
disp(hard.model.modelEscrit)
[hard.model.model,hard.model.species_list,hard.model.errstr] = eq2mat_hard(hard.model.modelEscrit); % gathers hard.model.model from eq2mat from mechanism string
...
modelActiu=evalin('base','mcr_als.alsOptions.kinetic.modelActiu;'); % Gathers model actiu??
% Set expression to [simula.eixx, simula.perfilsc]=ode15s()
% ode15s() solves stiff differential eqs
% Output:
%   eixx --> time points evaluated upon
%   perfilsc --> y solutions array
% Input
%   Uses gui_conc function: this is your y' or dy/dt function
%   time as tspan, models.model as y0, and options:
%       .concs.ci is 1 x num species with the starting concentrations as entered
%       
...
evalin('base',expressionX);
axes(handles.axes1);
% perfilc from linear interpolation of 
%   x --> eixx: the timepoint output of ode
%   v --> perfilsc: the y solutions of the ode
%   xq --> time_axis: values we want to know vq of
