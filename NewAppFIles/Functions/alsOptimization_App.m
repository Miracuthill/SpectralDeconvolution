...
matdad=evalin('base','mcr_als.data'); % This is the data matrix
nsign=evalin('base','min(size(mcr_als.alsOptions.iniesta))'); % The size of the inital estimate
iniesta=evalin('base','mcr_als.alsOptions.iniesta'); % Iniesta is the Initial Estimate
nexp=evalin('base','mcr_als.alsOptions.nexp'); % Number of sub matrices?
Fixed = evalin('base', 'mcr_als.InitEstim.Fixed'); % Bool - is there a fixed matrix? 0:none, 1:Fixed Spectra, 2:Fixed Concentration
[nrow,ncol]=size(matdad);
[nrow2,ncol2]=size(iniesta);

if nexp==1 % If no submatrices(only one matrix)
    isp=ones(nsign,1);
else % If Multiset, get defining characteristics
...
% Define from inital estimate size if estimate is column or row estimate
    % Redefine nsign as # components in initial estimate
if nrow2==nrow,	nsign=ncol2; ils=1;end % Rows are equal, thus estimate is column/concentration profile
if ncol2==nrow, nsign=nrow2; iniesta=iniesta'; ils=1; end % estimate col = num rows, thus flip estimate and still column/concentration profile

if ncol2==ncol, nsign=nrow2; ils=2;end % Columns are equal, thus estimate is row/spectra profile
if nrow2==ncol, nsign=ncol2; iniesta=iniesta'; ils=2; end % estimate row = num cols, thus flip estimate and still row/spectra profile

if ils==1 % For column/concentration estimate
    conc=iniesta;
    [nrow,nsign]=size(conc); % we already defined nsign, but okay...
    abss=conc\matdad; % calculate correlating row/spectra
end

if ils==2 % For row/spectra estimate
    abss=iniesta;
    [nsign,ncol]=size(abss);
    conc=matdad/abss; % Calculate correlating column/concentration
end

if nexp==1 % One matrix (no submatrices)
    % Set values that were assigned to multimatrix to prevent error
    ncinic(nexp)=1;
    ncfin(nexp)=ncol;
    nrinic(nexp)=1;
    nrfin(nexp)=nrow;
...
if nexp==1  % One matrix (no submatrices)
    nrsol(1)=nrow;
    nrinic(1)=1; 
    nrfin(1)=nrsol(1); % Number of rows in concentration mat
    nesp(1)=nsign;
    matr = 1;
    matc = 1;
...
% PCA or principal component analysis allows summarization of information
% through summary indices
% dn is the experimental matrix and d is the PCA reproduced matrix
d=evalin('base','mcr_als.data');
dn=d;
[u,s,v,d,sd]=pcarep(dn,nsign); % PCA reproduction of the original data matrix dn for nsign # of components
% u, s, v are from [u,s,v] =svd(dn)
% d is from u*s*v
% Basically deconstructs dn w/ svd and then reconstructs is based on nsign
% for dimensions
sstn=sum(sum(dn.*dn)); % sums all elements in dn^2
sst=sum(sum(d.*d)); % sums all elements in d^2
sigma2=sqrt(sstn); % sqrt of sum of dn^2 (like a magnitude?)
...
while niter < evalin('base','mcr_als.alsOptions.opt.nit'); 
    ...
    conc=d/abss; % calculate concentration from spectra
    
    % ******************************************
    % CONSTRAIN APPROPRIATELY THE CONCENTRATIONS
    % ******************************************
    
    if Fixed ~= 2 % Then constraint can be constrained

    % ****************
    % non-negativity
    % ****************
    
    if evalin('base','mcr_als.alsOptions.nonegC.noneg')==1; % If non negativity is applied to concentration
        ineg=evalin('base','mcr_als.alsOptions.nonegC.noneg');
        if ineg ==1; % check again? unnecesary?
            
            ialg=evalin('base','mcr_als.alsOptions.nonegC.ialg'); % implementation selectin
            ncneg=evalin('base','mcr_als.alsOptions.nonegC.ncneg'); % # species w/ nonneg profile
            cneg=evalin('base','mcr_als.alsOptions.nonegC.cneg'); % Vector of positive profiles
            
            for i=1:matc % For each concentration matrix (one if no sub matrices)
                kinic=nrinic(i);
                kfin=nrfin(i);
                conc2=conc(kinic:kfin,:);
                
                if ialg==0 % Force to zero
                    for k=1:nsign % For each component
                        if cneg(i,k) ==1 % If the profile is non-negativity (in vector of positive)
                            for j=1:kfin+1-kinic
                                if conc2(j,k)<0.0 % If neg
                                    conc2(j,k)=0.0; % Force to zero
                                end
                            end
                        end
                    end
                ...
                if ialg==1 % nnls (non-negative least squares)
                    for j=kinic:kfin
                        if cneg(i,:) == ones(1,size(isp,2)) % If all components of current conc matrix is in positive vector
                            x=lsqnonneg(abss',d(j,:)'); % Solves: minx||(abss')x - d(j,:)||(norm 2) where x>=0. uses transposed spectra and data
                            conc2(j-kinic+1,:)=x';
                        end
                    end
                end
                
                if ialg==2 % fnnls (fast non-negative least squares)
                    for j=kinic:kfin
                        if cneg(:,i) == ones(1,size(isp,2)) % Changed from cneg(i,:) 
                            x=fnnls(abss*abss',abss*d(j,:)'); % Function by GUI designers based off lsqnonneg().
                            conc2(j-kinic+1,:)=x';
                        end
                    end
                end
                
                conc(kinic:kfin,:) = conc2; % Apply new concentration probile to conc mat
            end
        end
    end
    ...
    if evalin('base','mcr_als.alsOptions.trilin.appTril')==1 % Is a trilinear model?
    ...
    if matc>1 % For multi set data
        for i=1:matc % For each sum matrix
            for j=1:nsign % For each component
                if isp(i,j)==0 % For zero concentration species
                    conc(nrinic(i):nrfin(i),j)=zeros(nrsol(i),1); % Set concentration to zero
                end
            end
        end
    end
    ...
    if evalin('base','mcr_als.alsOptions.unimodC.unimodal')==1 % Is it a unimodal concentration/column profiles?
        
        for i = 1:matc % For each submatrix
            
            kinic=nrinic(i);
            kfin=nrfin(i);
            conc2=conc(kinic:kfin,:); % Reproduce conc
            imod=evalin('base','mcr_als.alsOptions.unimodC.unimodal'); % Why do we need this again
%             imod=evalin('base','mcr_als.alsOptions.unimodc.')
            
            if imod==1|imod==3, % And wouldn't it either be 0 or 1 depending on if unimodal is checked or not 
                spmod=evalin('base','mcr_als.alsOptions.unimodC.spmod'); % Vector of unimodal profiles
                cmod=evalin('base','mcr_als.alsOptions.unimodC.cmod'); % Implementation: vertical or horizontal or average
                rmod=evalin('base','mcr_als.alsOptions.unimodC.rmod'); % Constraint tolerance (default: 1.0001)
                
                for ii=1:nsign % For each component
                    if spmod(i,ii)==1 % If the i submatrix and ii component is unimodal
                        conc2(:,ii)=unimod(conc2(:,ii),rmod,cmod); % Run function according to tolerance and implementation style
                    end
                end
                
            end
            
            conc(kinic:kfin,:)=conc2; % Apply new conc profile.
        end
    end
    ...
    if evalin('base','mcr_als.alsOptions.closure.closure')==1;
        ...
        dc=evalin('base','mcr_als.alsOptions.closure.dc'); % =2 ?
        vc=evalin('base','mcr_als.alsOptions.closure.vc');
        iclos=evalin('base','mcr_als.alsOptions.closure.iclos'); % number of closure constraints to include
        ...
        % this should be performed for all data slices
        for i=1:matc;
            if iclos(i)==1 % onle closure condition
                
                iclos1=evalin('base','mcr_als.alsOptions.closure.iclos1');
                sclos1=evalin('base','mcr_als.alsOptions.closure.sclos1'); % Edit box with which species are in closure 1
                
                if vc==0
                    tclos1=evalin('base','mcr_als.alsOptions.closure.tclos1'); % First closure constraint equal to
                else
                    vclos1=evalin('base','mcr_als.alsOptions.closure.vclos1'); % First variable closure
                end
                
            elseif iclos(i)==2 % 2 closure condition
                
                iclos1=evalin('base','mcr_als.alsOptions.closure.iclos1'); % type of closure condition
                ...
                if vc==0
                    tclos1=evalin('base','mcr_als.alsOptions.closure.tclos1'); % First closure constraint equal to
                    tclos2=evalin('base','mcr_als.alsOptions.closure.tclos2'); % Second closure constraint equal to
                else
        ...
        % end of the loop for all the data slices
    ...
    if evalin('base','mcr_als.alsOptions.correlation.appCorrelation')==1
        ...
        if nexp==1 
            [yout,ycal,stats]=yregrnew(conc,csel,compreg); % regression,  csel size must = conc size
            conc=yout;
            conc(iisel)=csel(iisel);
        end
        if nexp>1
        ...
        if nexp==1
            ...
            for i=1:nmodel
                ...
                for j=1:length(expkin)
                    ...
                    conckin{j,1}=interesp(conckin{j,1},Ccol); % Script for the incelation of coloured/non-coloured species arxiu 
                end
                if niter ==1
    ...
    end % End of Concentration iters
    % ********************************
    ...
    s1 = abss;
    if Fixed ~= 1 % Then changes can be made to Spectra
    ...
    if evalin('base','mcr_als.alsOptions.trilin.appTril')==1
        ...
        if ishape>=1,
            ...
            trildir=evalin('base','mcr_als.alsOptions.trilin.trildir');
            spetric=evalin('base','mcr_als.alsOptionsals_res_App.trilin.spetric');
            %             spetris=evalin('base','mcr_als.alsOptions.trilin.spetris');
    ...
    if evalin('base','mcr_als.alsOptions.unimodS.unimodal')==1
        ...
        for i = 1:matr
            ...
            % Origionally if imod==2|imod==3|, but why would it be not 0 or
            % 1... I changed to 1
            if imod==1
                spsmod=evalin('base','mcr_als.alsOptions.unimodS.spsmod'); % Whice species?
                cmod=evalin('base','mcr_als.alsOptions.unimodS.cmod'); % Implementation type 0-vertical, 1-horizontal, 2-average
                smod=evalin('base','mcr_als.alsOptions.unimodS.smod'); % Tolerance
    ...
    if evalin('base','mcr_als.alsOptions.sselcS.sselcon')==1
        ...
        ssel=evalin('base','mcr_als.alsOptions.sselcS.ssel'); % Spectral selectivity mat
        type_ssel=evalin('base','mcr_als.alsOptions.sselcS.type_ssel'); % Type is <=, =, >=
        jjsel=evalin('base','mcr_als.alsOptions.sselcS.jjsel'); % Finite indices in SSel
        
        if type_ssel==0 % If equality
            abss(jjsel)=ssel(jjsel); % Spectra of finite indices = selectivity mat of finite indices
        end
        
        if type_ssel==1 % If <= 
            for jj=1:size(jjsel),
                if abss(jjsel(jj))>ssel(jjsel(jj)) % If greater, become equal
        ...
        if type_ssel==2
            for jj=1:size(jjsel),
                if abss(jjsel(jj))<ssel(jjsel(jj)) % if less than, become greater
                    abss(jjsel(jj))=ssel(jjsel(jj));
                end
            end
        end
    ...
    if evalin('base','mcr_als.alsOptions.closure.closure')==1;
        ...
        vc=evalin('base','mcr_als.alsOptions.closure.vc');
        iclos=evalin('base','mcr_als.alsOptions.closure.iclos'); % number of closure constraints
        tclos1(1:matc)=zeros(1,matc);
        tclos2(1:matc)=zeros(1,matc);
        sclos1(1:matc,1:nsign)=zeros(matc,nsign);
        sclos2(1:matc,1:nsign)=zeros(matc,nsign);
    ...
    end % Finish Spectra Adjustments
    s2 = conc;
    ...
    if change < 0.0,
        disp(' ')
        disp('FITTING IS NOT IMPROVING !!!')
        idev=idev+1;
    else,
        disp('FITTING IS IMPROVING !!!')
        idev=0;
    end
...
%         end
    ...
    niter
    evalin('base','mcr_als.alsOptions.opt.nit')
end
