...
if ig(1,1)==1,
    ...
    gclosineq=abs(clos-closnew)-clostol;
...
if ig(1,5)==2,
   ...
   lsvd=svd(ctril);
   ls=lsvd(2)/lsvd(1);
   % lack of trilinearity tolerance
   % ls=ls-0.00001;
   % nsign equality constraints
   gtril=[gtril;ls];
   end
end        
...
% g_eq=[gnorm;gknown;gcloseq];
% g_ineq=[gcneg;gsneg;gsel;gunimod;gtril];
% nconstr=[length(gnorm),length(gknown),length(gcneg),length(gsneg),length(gsel),length(gunimod),length(gtril),length(gcloseq),length(g)];
g=[g_eq;g_ineq];
%  pause